...
class StockPicking(models.Model):
    ...
    need_approval = fields.Boolean("Need Approval", tracking=True, copy=False)
    ...
    man_confirm = fields.Boolean("Manager Confirmation", tracking=True)
    ...
    def action_line_manager_approval(self):
        ...
        group_id = self.env.ref("naseni_store_request.group_qa")
        for move in self.move_ids_without_package:
            move.state = "approve"
    ...
    def send_store_request_mail(self):
        if self.picking_type_id.name == "Staff Store Requests" and self.state in [
            "draft",
            "approve",
            "waiting",
            "confirmed",
    ...
    def send_store_request_done_mail(self):
        if self.state in ["done"]:
            subject = (
                "Store request '{}', for {} has been approved and validated".format(
                    self.name, self.employee_id.name
                )
            )
            partner_ids = []
    ...
    def create_atp_order(self):
        ...
        res = {
            ...
            "target": "current",
            "context": {
                "default_project_des": self.project_description,
                "default_stock_source": self.name,
                "default_expected_date": self.scheduled_date,
                "default_atp_form_line_ids": order_lines,
            },
        }
        return res
    ...
    def _notify_get_action_link(self, link_type, **kwargs):
        ...
        if self.picking_type_id == self.env.ref(
            "naseni_store_request.stock_picking_type_emp"
        ):
    ...
    def _get_record_url(self):
        ...
        params = {
            "id": self.id,
            "cids": self.id,
            "action": int(self.env.ref("naseni_store_request.store_req_action_window")),
            "model": self._name,
            "menu_id": int(self.env.ref("naseni_store_request.store_request")),
            "view_type": "form",
        }
